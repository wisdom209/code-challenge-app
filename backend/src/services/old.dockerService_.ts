...
import Task, { ITask, ITestCase } from '../models/Task';
...
class DockerService {
  ...
  private generatePythonTestScript(
    ...
    // Extract function name from entryPoint (e.g., "reverse_a_string.py" -> "reverse_a_string")
    ...
    testCases.forEach((tc, idx) => {
      // Format input and expected output as Python literals
  ...
  private generateCTestScript(
    ...
    const lines: string[] = [
      '#!/bin/bash',
      '',
      `gcc ${entryPoint} -o /tmp/solution 2>&1`,
      'if [ $? -ne 0 ]; then',
      ...
      '  EXPECTED="$2"',
      '  DESC="$3"',
      '  OUTPUT=$(echo -n "$INPUT" | /tmp/solution 2>&1)',
      '  EXIT_CODE=$?',
    ...
    testCases.forEach((tc, idx) => {
      const desc = tc.description || `Test ${idx + 1}`;
      // Escape quotes in input/output for bash
  ...
  async runTestsFromRepo(
    ...
    try {
      ...
      let testScriptContent: string;
      let testScriptFilename: string;
      if (language === 'python') {
        testScriptContent = this.generatePythonTestScript(
          taskConfig.testCases,
          taskConfig.entryPoint
        );
        testScriptFilename = 'test_solution.py';
      } else if (language === 'c') {
        testScriptContent = this.generateCTestScript(
          taskConfig.testCases,
          taskConfig.entryPoint
        );
        testScriptFilename = 'test_solution.sh';
      } else {
        throw new Error(`Unsupported language: ${language}`);
      }

      // Write the script to test directory
      const testScriptPath = path.join(testDir, testScriptFilename);
      await fs.writeFile(testScriptPath, testScriptContent);
      await fs.chmod(testScriptPath, 0o755); // make executable

      // 8. Define container options - mount the test directory
      const cmd =
        language === 'python'
          ? ['python', testScriptFilename]
          : ['bash', testScriptFilename];
      const containerOptions: ContainerCreateOptions = {
        ...
        Cmd: cmd, // Execute the test command
      };
